...
export class EspaceProfesseurComponent implements OnInit, AfterViewInit, OnDestroy {

  encadrementService=inject(EncadrementService);
  enseignantService=inject(EnseignantService);
  idEnseignantConnecte=0;
  nbrTotalEtudiants=0;
  nbrDepots=0;
  nbrDepotsValide=0;
  ...
  ngOnInit(): void {
    ...
    if(connectedUser){
      this.idEnseignantConnecte=JSON.parse(connectedUser).id;
    }

    this.encadrementService.getNombreTotalEtudiants(this.idEnseignantConnecte).subscribe((res:any)=>{
      this.nbrTotalEtudiants=res;
    });

    this.enseignantService.getNombreDepots(this.idEnseignantConnecte).subscribe((res:any)=>{
      this.nbrDepots=res;
      console.log(res)
    })

    this.enseignantService.getNombreDepotsValide(this.idEnseignantConnecte).subscribe((res:any)=>{
      this.nbrDepotsValide=res;
      console.log(res)
